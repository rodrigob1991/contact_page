...
type HandleUserSubscriptionToMessages<UT extends UserType> = UT extends "host" ? (sm: SendMessage<"host">) => HandleUserSubscriptionToMessagesReturn : (sm: SendMessage<"guess">, gi: number) => HandleUserSubscriptionToMessagesReturn
...
type CacheAndSendUntilAck<UT extends UserType> = UT extends "host" ?  <M extends OutboundMessage<"host">[]> (cache: boolean, key: RedisMessageKey<M>, message: M[number]["template"]) => Promise<void> : <M extends OutboundMessage<"guess">[]>(cache: boolean, key: RedisMessageKey<M>, message: M[number]["template"], guessId: number) => Promise<void>
