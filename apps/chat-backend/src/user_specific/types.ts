...
type CacheAndSendUntilAck<UT extends UserType> = UT extends "host" ?  <M extends OutboundMessage<"host">[]> (key: RedisMessageKey<M>, message: M[number]["template"]) => void : <M extends OutboundMessage<"guess">[]>(key: RedisMessageKey<M>, message: M[number]["template"], guessId: number) => void
