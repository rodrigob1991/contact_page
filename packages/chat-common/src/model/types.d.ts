import { emptyGuess, emptyHost, messageFlows, messageParts, messagePrefixesIn, messagePrefixesOut, oppositeUserTypes, userTypes } from "./constants";
import { ChangePropertiesType, IfAllIn } from "utils/src/types";
export type UserType = keyof typeof userTypes;
export type User<UT extends UserType = UserType> = {
    type: UT;
    data: ("host" extends UT ? Host : never) | ("guess" extends UT ? Guess : never);
};
...
export type MessagePrefix<MF extends MessageFlow = MessageFlow> = ("in" extends MF ? MessagePrefixIn : never) | ("out" extends MF ? MessagePrefixOut : never);
...
export type MessageParts = {
    ...
    [messageParts.originPrefix]: OriginPrefix;
    [messageParts.number]: number;
    [messageParts.userId]: number;
