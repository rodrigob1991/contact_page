import { MessageFlow, MessageParts, MessagePartsKeys, MessagePrefix, MessagePrefixExclusiveOut, OppositeUserType, OriginPrefix, UserType } from "../model/types";
import { IfOneIn } from "utils/src/types";
type SpecificMessagePartsKeys<UT extends UserType, MF extends MessageFlow, MP extends MessagePrefix<MF>, OP extends OriginPrefix<MF, MP>> = "prefix" | IfOneIn<OriginPrefix, OP, "originPrefix"> | "number" | IfOneIn<"con" | "dis" | "mes" | IfOneIn<Exclude<OriginPrefix, "usrs"> | "out", OP | MF, "uack"> | "sack", MP, "userId"> | IfOneIn<"mes" | "usrs" | "con" | "dis", MP, "body">;
...
type MessageTemplateInstance<MF extends MessageFlow, MP extends MessagePrefix<MF>, OP extends OriginPrefix<MF, MP>, MPKS extends MessagePartsKeys> = `${MP}${PartTemplate<"originPrefix", MPKS, ":", OP>}${PartTemplate<"number", MPKS, ":">}${PartTemplate<"userId", MPKS, ":">}${PartTemplate<"body", MPKS, ":">}`;
...
type MessageInstance<UT extends UserType, MF extends MessageFlow, MP extends MessagePrefix<MF>, OP extends OriginPrefix<MF, MP> = OriginPrefix<MF, MP>, SMPK extends SpecificMessagePartsKeys<UT, MF, MP, OP> = SpecificMessagePartsKeys<UT, MF, MP, OP>> = {
    userType: UT;
    flow: MF;
    prefix: MP;
    originPrefix: OP;
    ...
    template: MessageTemplateInstance<MF, MP, OP, SMPK>;
};
type MessageInstanceUserAck<UT extends UserType, OP extends OriginPrefix> = {
    [K in OP]: MessageInstance<UT, "in", "uack", K>;
}[OP];
...
export type GetMessages<UT extends UserType = UserType, MF extends MessageFlow = MessageFlow, MP extends MessagePrefix<MF> = MessagePrefix<MF>> = [
    ...
    ...FilterMessage<OutboundToHostGuessesMessage, UT, MF, MP>,
    ...FilterMessage<OutboundToGuessHostsMessage, UT, MF, MP>,
...
export type GotAllMessageParts<M extends Message> = {
    [K in keyof M["positions"]]: K extends "prefix" ? M["prefix"] : K extends "originPrefix" ? M["originPrefix"] : K extends MessagePartsKeys ? MessageParts[K] : never;
};
export {};
